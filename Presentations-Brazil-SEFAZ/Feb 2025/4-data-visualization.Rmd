...
.pull-left[
- `mes_emissao` is a variable representing months but R doesn't know it and it's showing simply the numbers

- We can center the title

- We should add axis labels (instead of just "mes_emissao" and "vicms") 

- Too much information on background: grey color, lines
]
...
Use the following code to create a dataframe collapsed at the month-issuer level and calculate the total ICMS liability for each issuer in each month
...
We might make it better by doing some adjustments: **make the legend labels, titles and x-axis texts smaller** and to **label the legend title** 
...
1. Add the argument `legend.title=element_text(size=7)` inside `theme()` to decrease the legend text size
...
1. Add the line `labs(color = "Emissor")` to change the title of the legend
...
- We use the encoding `geom_point()` for scatterplots
