...
## Exercise 1b: Load the data we'll use
...
You should have these two dataframes loaded in the environment after this.
...
```{r eval= FALSE}
# Filter only NFes from December:
temp1 <- filter(base_nfe, mes_emissao == 12)

# Sort previous result by value, descending order:
temp2 <- arrange(temp1, -vprod)

# Keep only the 50 first items after sorting:
...
This code works, but the problem with it is that it is cumbersome to write and makes us generate unnecessary intermediate dataframes (`temp1`, `temp2`) that store results temporarily
...
Create a summary statistics table showing the number of observations, mean, standard deviation, minimum, and maximum for variables `vprod`, `vicms` of the dataframe `base_nfe`
...
```{r eval=FALSE}
datasummary(
 vprod + vicms ~ N + Mean + SD + Min + Max,  # this is the formula
  base_nfe                                   # this is the data
)
```
