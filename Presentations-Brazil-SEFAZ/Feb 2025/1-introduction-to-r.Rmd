...
1. Começa com uma entrada (*input*) de dados
2. Realiza algum processo com os dados
3. Produz resultados (*output*)
...
## Como pode me beneficiar?


.pull-left[
- **Reproducibilidade**: Códigos registram cada passo e decisão tomada na análise de dados, fazendo com que resultados possam ser compreendidos e verificados

- **Reutilização**: Código pode ser reutilizado para projetos similares, poupando tempo em atividades futuras

- **Transparência em comparação com Excel**: Instruções são documentadas e menos propensas a erros que passos manuais no Excel
]
...
- Algumas razões pra usar R:
  ...
  + Ótimas recomendações do *ChatGPT* & outros LLM
  + Versatilidade: esses slides foram produzidos em R(Markdown)!
]
...
- Criamos objetos no R com o operador de seta (`<-`) ou com operador de igual (`=`)
...
## Exercício 3: Carregando dados no R
 ...
 1.- Verifique que você tem acesso a estes arquivos:
...
## Exercício 3: Carregando dados no R
 ...
 2.No RStudio, vá em `File` > `Import Dataset` > `From Text (base)` e selecione o arquivo  `base_nfe.csv`
...
- Você pode clicar no `x` o lado de `base_nfe` para voltar ao script 
- Para abrir o visualizador novamente, use o código: `View(base_nfe)` (note o "V" maiúsculo)
...
- Além disso, agora você verá um objeto chamado `base_nfe` no seu painel de ambiente
...
- Lembra que mencionamos os objetos antes? Para o R, `base_nfe` é um objeto assim como `x1`, `x2`, ou `x3`

- A diferença é que `base_nfe` não é um único número como `x1`, mas uma coleção de valores numéricos semelhantes a uma planilha do Excel. No R, esse tipo de objeto é chamado **dataframes**
...
- Como os dataframes também são objetos, podemos nos referir a eles pelos seus nomes (exm: `base_nfe`)
...
Compreender os dados que você usa é muito importante. Para este treinamento, `base_nfe` é um dataframe de exemplo com dados de items de Notas Fiscais Eletronicas para Out-Dez 2024:

<details>
<summary><b>Colunas ou Variáveis</b></summary>

- `nfuid_anon` é o identificador da NFe
- `dt_emissao` e `mes_emissao` indicam datas de emissão
- `emitcnpj9_anon` é identificador anonimizado de CNPJ 8 digitos
- `xprod` é descrição do item
- `vprod` e `vicms` são valores de venda e ICMS
- `cfop` é Código Fiscal de Operações e Prestações
...
1. Use o seguinte código para criar um subconjunto de `base_nfe` mantendo apenas as observações de NFe emitidas em Outubro (mês 10): `base_outubro = subset(base_nfe, mes_emissao == 10)`

  + Note que estamos usando o operador de igual (`=`) para armazenar o resultado
  + Note que há **dois sinais de igual** na condição, não um
  
2. Use `View(base_outubro)` para visualizar o dataframe novamente e ver como ele mudou (note a letra maiúscula "V")
...
Há uma diferença importante entre usar  `<-` ou `=` e não usá-los
...
```{r eval=FALSE}
subset(base_nfe, mes_emissao == 10)
```
...
- Usar `=` informa ao R que queremos armazenar o resultado em **um novo objeto**, que é o objeto à esquerda da seta. Dessa vez, o resultado não será impresso no console, mas o novo dataframe aparecerá no painel de ambiente

```{r echo=FALSE}
base_nfe <- fread('/Users/tscot/Downloads/Treinamento_SEFAZ/base_nfe.csv')
```

```{r}
base_outubro = subset(base_nfe, mes_emissao == 10)
```
...
```{r echo = FALSE, out.width="60%"}
knitr::include_graphics("img/session1/excel-tabs2.png")
```
...
- **Dataframe:** uma coleção de valores organizados em linhas e colunas. `base_nfe` and `base_outubro` são dataframes. Eles são semelhantes a uma planilha do Excel.

No próximo exercício, aprenderemos sobre outro tipo de objeto. **(para consulta própria)**
...
1.- Operar com dois vetores aplica a operação **elemento por elemento** 
...
- O resultado de uma função pode sempre ser armazenado em um objeto utilizando o operador de seta (`<-`) ou igual (`=`)
...
1. Calcule as estatísticas descritivas das variáveis de  `base_nfe` e armazene o resultado com `summary_table <- summary(base_nfe)`
...
Observe que este código está tanto criando um novo objeto (com `summary_table <- summary(base_nfe)`) quanto imprimindo o resultado no console (com `print(summary_table)`)
